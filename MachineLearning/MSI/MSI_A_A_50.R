...
set.seed(125)
train.index <- createDataPartition(MSI_TVC_A$TVC, p = .7,list = FALSE, groups=3, times = 50)
...
RMSELMT<-c()
for (i in 1:50) {
  trainN <- MSI_TVC_A[train.index[,i],]
  testN <- MSI_TVC_A[-train.index[,i],]
...
set.seed(125)
train.indexs <- createDataPartition(MSI_PS_A$Pseudomonas.spp., p = .7, list = FALSE,groups = 3, times = 50)
...
RMSELMP<-c()
for (i in 1:50){
  trainNP <- MSI_PS_A[train.indexs[,i],]
  testNP <- MSI_PS_A[-train.indexs[,i],]
...
##################################################################################
################ k-nearest neighbours for PS count by  MSI_data #################
##################################################################################
RMSESKNNP<-c()
...
###################################################################################
#########################SVMRadial for PS count by MSI_data#######################
###################################################################################
RMSESRADP<-c()
