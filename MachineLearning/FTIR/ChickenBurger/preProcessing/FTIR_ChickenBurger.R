...
library("readxl")
library("mixOmics")
library(limma) # to remove batch effect
library(factoextra)
library(ggbiplot)
...
# #remove outliers
# outliers <- boxplot(dfM$STAA, plot=FALSE)$out
# dfM2 <- dfM[-which(dfM$STAA %in% outliers),]
# 
...
#   if(dfM2$STAA[i] < 4.50){
#     bc_outNum3 = bc_outNum3 + 1
#     id_outNum3[length(id_outNum3)+1] <- i
#   }
# }
# id_outNum3 <- id_outNum3[-1]
# row.names(dfM2[21,]) #DBB0_119
# 
# # draw boxplot with removed outliers
# outliers3 <- boxplot(dfM2$STAA, plot=FALSE)$out
# dfM3 <- dfM2[-which(dfM2$STAA %in% outliers3),]
# boxplot(dfM3[,1:4], 
#         xlab = "medium",
#         ylab = "bacterial counts") 
# title("BACTERIAL COUNTS - BOXPLOT WITH REMOVED OUTLIERS \n(the second elimination)")
...
#   if(dfM3_names[i]=="12BA12_27"){
#     rowID[length(rowID)+1]<- i
#   } else if(dfM3_names[i]=="12BA93_34"){
#     rowID[length(rowID)+1]<- i
#   } else if(dfM3_names[i]=="16BA12_40"){
#     rowID[length(rowID)+1]<- i
#   } else if(dfM3_names[i]=="16BB84_49"){
#     rowID[length(rowID)+1]<- i
#   } else if(dfM3_names[i]=="16BB93_50"){
#     rowID[length(rowID)+1]<- i
#   }
# }
# dfM4 <- dfM3
...
# #group data again 
# batchNum4 <- c()
# dfM4_names <- row.names(dfM4)
# for(i in 1:length(dfM4_names)){
#   batchNum4[i] <- sub("\\d*B|DB", "", dfM4_names[i])
#   batchNum4[i] <- substr(batchNum4[i], 1, 1)
# }
# #pca based on data with removed outliers
# pca_dfM4 <- prcomp(dfM4[,5:3355]) 
# 
# #grouped PCA scores plot 
# # grouped based on batch and condition
# # PC1 vs PC2
# fviz_pca_ind(pca_dfM4, axes = c(1,2), geom = c("point", "text"), 
#              col.ind = batchNum4, addEllipses = TRUE) 
# # PC2 vs PC3
# fviz_pca_ind(pca_dfM4, axes = c(2,3), geom = c("point", "text"), 
#              col.ind = batchNum4, addEllipses = TRUE) 
# 
# # PC1 vs PC3
# fviz_pca_ind(pca_dfM4, axes = c(1,3), geom = c("point", "text"), 
#              col.ind = batchNum4, addEllipses = TRUE) 
...
# ###########################################################################################################
# library(writexl)
# dfM6_samplesNames <- row.names(dfM6) 
# dfM6_samplesNames <- as.matrix(dfM6_samplesNames )
# row.names(dfM6_samplesNames) <- dfM6_samplesNames[,1]
# dfM6_withSampleNames <- CreateObj(dfM6_samplesNames, dfM6)
# write_xlsx(dfM6_withSampleNames, path = "chickenBurger_FTIR_preprocessed.xlsx", col_names = TRUE)
# 
# ###############################################
# # BASIC STATISTICS ############################
# ###############################################
...
# spectraLogTrans <- log2(spectra + 1)
# 
...
# obj1 <- CreateObj(samples, spectra)
# #set samples names
# rownames(obj1) <- obj1[,1]
# obj1 <- obj1[,-1]
# 
...
# # create groups based on conditions and batches
# groups <- c(rep("1st batch, isothermal storage", 306), rep("2nd batch, isothermal storage", 324),
#             rep("1st batch, dynamic storage", 78), rep("2nd batch, dynamic storage", 66))
# # create groups based on batches
# batch <- c(rep("1st batch", 306), rep("2nd batch", 324),
#            rep("1st batch", 78), rep("2nd batch", 66))
# 
# ##########################################################
# # pca based on raw spectra 
# pca <- prcomp(obj1, center = TRUE, scale = TRUE) # for raw spectra
# 
# #grouped PCA scores plot 
# # grouped based on batch and condition
# # PC1 vs PC2
# fviz_pca_ind(pca, axes = c(1,2), geom = c("point", "text"), 
#              col.ind = groups, addEllipses = TRUE) + xlim(-200, 350) 
# # scaled to identify seperated sample labels
# fviz_pca_ind(pca, axes = c(1,2), geom = c("point", "text"), 
#              col.ind = groups, addEllipses = FALSE, repel = TRUE) + xlim(300, 350) + ylim (10, 75)
# 
# # PC2 vs PC3
# fviz_pca_ind(pca, axes = c(2,3), geom = c("point", "text"), 
#              col.ind = groups, addEllipses = TRUE) + xlim(-320, 100)
# 
# # PC1 vs PC3
# fviz_pca_ind(pca, axes = c(1,3), geom = c("point", "text"), 
#              col.ind = groups, addEllipses = TRUE) + xlim(-200, 350)
# # scaled to identify seperated sample labels
# fviz_pca_ind(pca, axes = c(1,3), geom = c("point", "text"), 
#              col.ind = groups, addEllipses = FALSE) + xlim(312, 340) + ylim(-37, 2)
# fviz_pca_ind(pca, axes = c(1,3), geom = c("point", "text"), 
#              col.ind = groups, addEllipses = FALSE) + xlim(100, 200) + ylim(25, 37)
# 
...
# 
# ##########################################################
# # pca based on log-transform spectra 
# pcaTrans <- prcomp(objTrans[,6:3355], center = TRUE, scale = TRUE) # for log transformed spectra
# 
# # PC1 vs PC2
# ggbiplot(pcaTrans, choices = c(1,2), obs.scale = 1, var.scale = 1, 
#          groups = groups, ellipse = TRUE, 
#          circle = TRUE) + scale_color_discrete(name = '') + theme(legend.direction = 'horizontal', 
#                                                                   legend.position = 'top') 
# 
# # PC2 vs PC3
# ggbiplot(pcaTrans, choices = c(2,3), obs.scale = 1, var.scale = 1, 
#          groups = groups, ellipse = TRUE, 
#          circle = TRUE) + scale_color_discrete(name = '') + theme(legend.direction = 'horizontal', 
#                                                                   legend.position = 'top')
# 
# # PC1 vs PC3
# ggbiplot(pcaTrans, choices = c(1,3), obs.scale = 1, var.scale = 1, 
#          groups = groups, ellipse = TRUE, 
#          circle = TRUE) + scale_color_discrete(name = '') + theme(legend.direction = 'horizontal', 
#                                                                   legend.position = 'top')
# #grouped PCA scores plot (PC1 vs PC2)
# # grouped based on batch and condition
# fviz_pca_ind(pcaTrans, axes = c(1,2), geom = c("point", "text"), col.ind = groups, addEllipses = TRUE)
# # grouped based on batch
# fviz_pca_ind(pcaTrans, axes = c(1,2), geom = c("point", "text"), col.ind = batch, addEllipses = TRUE)
# 
# #scree plot
# fviz_eig(pcaTrans, addlabels = TRUE)
# 
# ######################################
# # pca for data with removed replicates
# # create groups based on conditions and batches
# groups <- c(rep("1st batch, isothermal storage", 306/6), rep("2nd batch, isothermal storage", 324/6),
#             rep("1st batch, dynamic storage", 78/6), rep("2nd batch, dynamic storage", 66/6))
# # create groups based on batches
# batch <- c(rep("1st batch", 306), rep("2nd batch", 324),
#            rep("1st batch", 78), rep("2nd batch", 66))
# # pca based on raw spectra 
# objM <- CreateObj(samplesNamneM, spectraM)
# #set samples names
# rownames(objM) <- objM[,1]
# objM <- objM[,-1]
# #pca
# pcaM <- prcomp(objM, center = TRUE, scale = TRUE) # for raw spectra
# # PC1 vs PC2
# fviz_pca_ind(pcaM, axes = c(1,2), geom = c("point", "text"),
#              col.ind = groups, addEllipses = TRUE) + xlim(-200, 350)x

# ###################################################
# ############## Outliers handling ##################
# ###################################################
# library(EnvStats)
# rosnerTest(spectraM, k = 7, warn = F)
# 
# boxplot(spectraM)
# ggplot(spectraM) + 
...
# 
# library (hyperSpec)
# waveLength <- c(as.numeric(colnames(spectraM)))
# spc <- new ("hyperSpec", spc = spectraM, wavelength = waveLength, data = spectraM)
# plotspc(spc)
# #does not work
# library(baseline)
# spcCorr <- baseline.modpolyfit(as.matrix(spectraM))
# spcCorr <- new ("hyperSpec", spc = spcCorr$corrected, wavelength = waveLength, data = spcCorr)
# plot(spcCorr)
# 
# 
# ###################################################
...
# 
# ###################################################
# # based on raw spectra data
# # create objective
# obj2 <- CreateObj(batch, obj)
# # remove batch effect
# objRBE_t <- removeBatchEffect(x = t(obj2[,2:3356]), batch = obj2$x)
# # transpose the data
# objRBE <- t(objRBE_t)
# # pca
# pca2 <- prcomp(objRBE[,5:3355], center = TRUE, scale = TRUE)
# fviz_pca_ind(pca2, axes = c(1,2), geom = c("point", "text"), col.ind = groups, addEllipses = TRUE)
# fviz_pca_ind(pca2, axes = c(1,2), geom = c("point", "text"), col.ind = batch, addEllipses = TRUE)
...
###########################################################################################################
library(writexl)
# raw data with removed one outlier version 1
...
# sg on normatised data version 1
...
### raw data with removed one outlier
# batch A
